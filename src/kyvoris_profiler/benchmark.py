...
def profile_callable(
    ...
    if not latencies_ms:
        raise RuntimeError("all measured iterations failed")
...
async def benchmark_async_callable(
    callable_obj: Callable[[], Awaitable[T]],
    iterations: int = 10,
    warmup: int = 0,
) -> ProfileSummary:
    """Measure the execution time of a no-argument async callable."""
    return await profile_async_callable(
        callable_obj,
        iterations=iterations,
        warmup=warmup,
    )
...
async def profile_async_callable(
    callable_obj: Callable[[], Awaitable[T]],
    iterations: int = 10,
    warmup: int = 0,
    collect_memory: bool = False,
    ...
    """Profile a no-argument async callable with optional resource metrics."""
    if iterations <= 0:
        raise ValueError("iterations must be greater than 0")
    if warmup < 0:
        raise ValueError("warmup must be greater than or equal to 0")
